...
#
def getRotx(deg):
    rx = np.mat([[1, 0, 0], [0, np.cos(deg), -np.sin(deg)],
                 [0, np.sin(deg), np.cos(deg)]])
    return rx


def getRoty(deg):
    ry = np.mat([[np.cos(deg), 0, np.sin(deg)], [0, 1, 0],
                 [-np.sin(deg), 0, np.cos(deg)]])
    return ry


def getRotz(deg):
    rz = np.mat([[np.cos(deg), -np.sin(deg), 0], [np.sin(deg),
                                                  np.cos(deg), 0], [0, 0, 1]])
    return rz
...
class Pose_t():
    ...
    def M(self):
        return np.dot(np.dot(getRotx(self.rx), getRoty(self.ry)),
                      getRotz(self.rz))
...
def clacRef(p1, p2, p3):
    ...
    rotation = np.transpose(
        np.array([
            np.transpose(coor_x.p),
            np.transpose(coor_y.p),
            np.transpose(coor_z.p)
        ]))
...
if __name__ == "__main__":
    ...
    # 计算夹具,随意的两个点
    p2 = np.array([[385.403, 97.112, 0], [0, 0, 0]])
    p1 = np.array([[286.933, 21.037, 0], [0, 0, np.pi]])
    # 开始计算
    # 将输入点换算为计算点
