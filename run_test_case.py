...
for i in range(1,2):
    ...
    discover = unittest.defaultTestLoader.discover(test_dir, pattern='gest*.py')
    # 定义带有当前测试时间的报告
    ...
    fp = open(filename, 'wb')
    # 定义测试报告
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title=u'websocket接口自动化测试报告/V_2.6.新核心板', description=u'用例执行情况')
    runner.run(discover)
    fp.close()